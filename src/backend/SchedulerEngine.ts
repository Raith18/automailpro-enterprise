...
const SchedulerEngine = (() => {
  function runAll(): void {
    ...
    try {
      ...
      for (const scheduler of schedulers) {
        if (scheduler.status !== 'active') continue;
  ...
  function runScheduler(scheduler: any, settings: any): void {
  ...
  function runNow(schedulerId: string): { sent: number; failed: number } {
    const scheduler = AppStorageManager.getSchedulers().find((s: any) => s.id === schedulerId);
    if (!scheduler) throw new Error('Scheduler not found: ' + schedulerId);
    const settings = AppStorageManager.getSettings();
    const rows = SpreadsheetManager.getTableData(scheduler.tabName, scheduler.tableStartRow);
    const tableBlob = SpreadsheetManager.getTableImageBlob(scheduler.tabName, scheduler.tableStartRow);
